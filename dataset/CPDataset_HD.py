...
class CPDataset(data.Dataset):
    """
        Dataset for CP-VTON.
    """
    def __init__(self, root, datamode, data_list, size=[512,384], semantic_nc=13, random_pair=False):
        super(CPDataset, self).__init__()
        # base setting
        self.root = root
        self.datamode = datamode # train or test or self-defined
        self.data_list = data_list
        self.fine_height = size[0]
        self.fine_width = size[1]
        self.semantic_nc = semantic_nc
        self.data_path = osp.join(root, datamode)
        self.with_parse_agnostic = True
    ...
    def __getitem__(self, index):
        ...
        # load_dino_fea
        #fea_name = im_name.replace('image', 'dino_fea').replace('.jpg', '.pt')
...
class CPDatasetTest(data.Dataset):
    ...
    """
    def __init__(self, root, datamode, data_list, size=[512,384], semantic_nc=13, datasetting='unpaired'):
    ...
    def __getitem__(self, index):
        ...
        # load background
        foreground_name = im_name.replace('.jpg', '_mask.png')
        foreground = Image.open(osp.join(self.data_path, 'foreground', foreground_name))
        foreground = transforms.Resize(self.fine_width, interpolation=2)(foreground)
        foreground = np.array(foreground)
        foreground_mask = torch.tensor(foreground / 255)
        ...
        parse_other = new_parse_map[1] + new_parse_map[2] + new_parse_map[4] + new_parse_map[7] + new_parse_map[8] + \
                            new_parse_map[9] + new_parse_map[10] + new_parse_map[11]+ new_parse_map[12]
        # dilated_pcm = cv2.dilate(pcm.squeeze(0).numpy(),kernel=np.ones((10, 10), np.uint8),iterations=1)
        # dilated_pcm = torch.tensor(dilated_pcm)
        parse_other = parse_other + torch.tensor(np.array(hand_mask))
        parse_other = parse_other.clamp(0,1)
        im_other = im * parse_other
        parse_upper_mask = (1 - parse_other).unsqueeze(0)
        ...
        # load_dino_fea
        fea_name = c_name[self.datasetting].replace('.jpg', '.pt')
...
if __name__ == '__main__':
    ...
    # config = OmegaConf.load('../train_tryon.yaml')
    config = OmegaConf.load('../infer_tryon.yaml')
    train_data_config = config.train_data
    dataset = CPDatasetTest(**train_data_config)
    ...
    high_frequency_map = p['high_frequency_map']['unpaired'].permute(1,2,0).numpy()
