...
class AnimatePipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        reference_control_writer = ReferenceNetAttention(self.referencenet, do_classifier_free_guidance=do_classifier_free_guidance, mode='write', fusion_blocks=self.fusion_blocks, batch_size=batch_size, is_image=False)
        reference_control_reader = ReferenceNetAttention(self.unet, do_classifier_free_guidance=do_classifier_free_guidance, mode='read', fusion_blocks=self.fusion_blocks, batch_size=batch_size, is_image=False)
        ...
        image = [image[i:i+frame] for i in range(0, len(image), frame)]
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, frame=1, latents=None):
        shape = (batch_size, num_channels_latents, frame, height // self.vae_scale_factor, width // self.vae_scale_factor)
