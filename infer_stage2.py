...
from pipelines.pipeline_stage2 import AnimatePipeline
...
config = OmegaConf.load('config/infer_stage2.yaml')
...
pipe = AnimatePipeline(pose_guider=pose_guider, referencenet=referencenet, vae=vae, unet=unet, scheduler=scheduler)
...
for i, batch in enumerate(test_dataloader):
    ...
    for idx, edited_image in enumerate(edited_images):
        frame_idx = 0
        for video_frame in edited_image:
            video_frame = torch.tensor(np.array(video_frame)).permute(2,0,1) / 255.0

            grid = make_image_grid([(pixel_values[idx][frame_idx].cpu() / 2 + 0.5),video_frame.cpu(), (pixel_values_pose[idx][frame_idx].cpu() / 2 + 0.5), (pixel_values_ref_img[idx].cpu() / 2 + 0.5)], nrow=2)
            save_image(grid, os.path.join(out_dir, (f'{image_idx}_{frame_idx}.jpg')))
