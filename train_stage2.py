...
def main():
    args = OmegaConf.load('config/train_stage2.yaml')
    print(args)
    ...
    referencenet = ReferenceNet.from_pretrained(args.stage1_model_path, subfolder="referencenet")

    poseguider_state_dict = torch.load(args.poseguider_checkpoint_path, map_location="cpu")
    poseguider.load_state_dict(poseguider_state_dict, strict=False)

    unet = UNet3DConditionModel.from_pretrained_2d(
        args.stage1_model_path, subfolder="unet", 
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            for i in range(len(models)):
                ...
                if i==0:
                    load_model = UNet3DConditionModel.from_pretrained_2d(
    ...
    poseguider.to(accelerator.device, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step_, batch in enumerate(train_dataloader):
            ...
            if not args.image_finetune:
                ...
                encoder_hidden_states = repeat(dino_fea, 'b n c -> (b f) n c', f=video_length)
            with accelerator.accumulate(unet):
                ...
                mask_dino = mask.unsqueeze(1).unsqueeze(2).expand_as(dino_fea)
                dino_fea[mask_dino] = 0
                mask_hidden = repeat(mask.unsqueeze(1).unsqueeze(2), 'b n c -> (b f) n c', f=video_length).expand_as(encoder_hidden_states)
                encoder_hidden_states[mask_hidden] = 0
                ...
                # torch.Size([12, 4, 64, 64]) torch.Size([12]) torch.Size([12, 50, 768]) 
                # torch.Size([1, 4, 64, 64]) torch.Size([1]) torch.Size([16, 50, 768])
                # print(latents_ref_img.shape, ref_timesteps.shape, encoder_hidden_states.shape)
                referencenet(latents_ref_img, ref_timesteps, dino_fea)
